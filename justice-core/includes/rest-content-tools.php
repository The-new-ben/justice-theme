<?php
/**
 * Justice Core ג€” REST Content Tools
 *
 * Routes:
 *   GET /wp-json/justice-core/v1/content/lawyers
 *   GET /wp-json/justice-core/v1/content/posts
 *   GET /wp-json/justice-core/v1/content/leads
 *   POST /wp-json/justice-core/v1/content/update-meta
 *   POST /wp-json/justice-core/v1/content/trash-post
 *
...
add_action( 'rest_api_init', 'uje_register_content_routes' );

function uje_register_content_routes(): void {
	$admin_only = function () {
		return current_user_can( 'manage_options' );
	};
	$write_allowed = function () {
		return current_user_can( 'manage_options' ) && (bool) apply_filters( 'uje_enable_rest_content_writes', false );
	};

	// Read routes
	register_rest_route( 'justice-core/v1', '/content/lawyers', array(
		'methods'             => 'GET',
		'callback'            => 'uje_content_lawyers',
	...
	register_rest_route( 'justice-core/v1', '/lawyers', array(
		'methods'             => 'GET',
		'callback'            => 'uje_content_lawyers',
		'permission_callback' => $admin_only,
		'args'                => array(
	...
	register_rest_route( 'justice-core/v1', '/content/posts', array(
		'methods'             => 'GET',
		'callback'            => 'uje_content_posts',
		'permission_callback' => $admin_only,
		'args'                => array(
			'per_page' => array( 'type' => 'integer', 'default' => 50, 'minimum' => 1, 'maximum' => 200 ),
			'page'     => array( 'type' => 'integer', 'default' => 1, 'minimum' => 1 ),
	...
	register_rest_route( 'justice-core/v1', '/content-inventory', array(
		'methods'             => 'GET',
		'callback'            => 'uje_content_posts',
		'permission_callback' => $admin_only,
		'args'                => array(
	...
	register_rest_route( 'justice-core/v1', '/content/leads', array(
		'methods'             => 'GET',
		'callback'            => 'uje_content_leads',
		'permission_callback' => $admin_only,
	) );

	register_rest_route( 'justice-core/v1', '/leads', array(
		'methods'             => 'GET',
		'callback'            => 'uje_content_leads',
		'permission_callback' => $admin_only,
	) );
	...
	// Write routes
	register_rest_route( 'justice-core/v1', '/content/update-meta', array(
		'methods'             => 'POST',
		'callback'            => 'uje_update_meta',
	...
	register_rest_route( 'justice-core/v1', '/content/trash-post', array(
		'methods'             => 'POST',
		'callback'            => 'uje_trash_post',
		'permission_callback' => $write_allowed,
		'args'                => array(
			'post_id' => array( 'type' => 'integer', 'required' => true ),
...
function uje_content_lawyers( WP_REST_Request $request ): WP_REST_Response {
...
function uje_content_posts( WP_REST_Request $request ): WP_REST_Response {
...
function uje_content_leads( WP_REST_Request $request ): WP_REST_Response {
	$query = new WP_Query( array(
	...
	foreach ( $query->posts as $post ) {
		$rows[] = array(
			...
			'title'   => $post->post_title,
			'date'    => $post->post_date,
			'name'    => get_post_meta( $post->ID, 'visitor_name',  true ),
			'phone'   => get_post_meta( $post->ID, 'visitor_phone', true ),
			'area'    => get_post_meta( $post->ID, 'legal_area',    true ),
			'city'    => get_post_meta( $post->ID, 'city',          true ),
			'source'  => get_post_meta( $post->ID, 'source_url',    true ),
		);
	}
 ...
 */
function uje_update_meta( WP_REST_Request $request ): WP_REST_Response {
	...
	uje_log( 'update_meta', "Updated {$meta_key} on post #{$post_id}", array(
		'post_id'  => $post_id,
		'meta_key' => $meta_key,
		'old_val'  => $old_val,
		'new_val'  => $meta_val,
 ...
 */
function uje_trash_post( WP_REST_Request $request ): WP_REST_Response {
	$post_id = absint( $request->get_param( 'post_id' ) );
	...
	uje_log( 'trash_post', "Trashed post #{$post_id}: {$post->post_title}", array(
		'post_id' => $post_id,
		'title'   => $post->post_title,
		'type'    => $post->post_type,
		'reason'  => $reason,
