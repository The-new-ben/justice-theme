 ...
 *
 * Two modes:
 * 1. Admin-init auto-seed: creates 10 demo lawyers once (uj_seeded_v4 flag).
 * 2. CSV seeder via REST POST /seed-lawyers — reads project-control/lawyer-seed.csv.
 *
 * REST: POST /wp-json/ultra-justice/v1/seed-lawyers (admin only)
 *
 * @package JusticeCore
 */
...
function uj_maybe_auto_seed(): void {
	...
	update_option( 'uj_seeded_v5', true );
	uj_log( 'seeder', 'Auto-seeded 10 lawyer profiles (v5).' );
}

// ─── CSV Seeder (from project-control/lawyer-seed.csv) ───────────────────────

function uj_seed_lawyer_profile( array $row ): int|WP_Error {
	$existing = get_posts( array(
		'post_type'      => 'justice_lawyer',
		'title'          => $row['name'],
		'posts_per_page' => 1,
...
function uj_run_seed(): array {
	$csv_path = uj_DIR . '../project-control/lawyer-seed.csv';
	if ( ! file_exists( $csv_path ) ) {
		$csv_path = uj_DIR . 'data/lawyer-seed.csv';
	}
	if ( ! file_exists( $csv_path ) ) {
	...
	while ( ( $data = fgetcsv( $handle ) ) !== false ) {
		...
		$result = uj_seed_lawyer_profile( $row );
	...
	fclose( $handle );
	uj_log( 'seeder_csv', 'CSV seed completed.', $results );
	return $results;
}

// REST endpoint for manual seeding
add_action( 'rest_api_init', function () {
	register_rest_route( 'ultra-justice/v1', '/seed-lawyers', array(
		'methods'             => 'POST',
		'callback'            => function () {
	...
	register_rest_route( 'ultra-justice/v1', '/seed-reset', array(
		'methods'             => 'POST',
		'callback'            => function () {
			delete_option( 'uj_seeded_v5' );
			uj_log( 'seeder_reset', 'Seed flag cleared. Will re-seed on next admin_init.' );
