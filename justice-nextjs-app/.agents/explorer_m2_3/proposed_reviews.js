...
function readLocalCache() {
  try {
    if (!fs.existsSync(CACHE_FILE_PATH)) {
      ...
      fs.writeFileSync(CACHE_FILE_PATH, JSON.stringify(MOCK_SEED_REVIEWS, null, 2), 'utf-8');
...
function writeLocalCache(reviews) {
  try {
    // Ensure directory exists (useful if writing to a subfolder like tmp/ or if structure is missing)
