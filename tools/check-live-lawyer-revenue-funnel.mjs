...
const checks = [
  {
    id: 'home-user-first-with-secondary-lawyer-path',
    name: 'Homepage stays user-first while exposing a quiet lawyer path',
    url: '/',
    type: 'page',
    required: [
      '/lawyers/',
      'id="find-lawyer-guide"',
      '/lawyer-registration/',
      '/lawyer-plans/',
      'lawyer-cta--quiet',
      'lawyer_profile_review',
      'homepage_quiet_lawyer_cta',
    ],
    absent: [
      'homepage-lawyer-revenue__account-steps',
      'pricing_card_lead_partner',
      'outreach_segment=homepage_lawyer_revenue',
      'lawyer_acquisition',
    ],
  },
  {
  ...
  },
  {
    ...
    absent: ['lawyer-registration-form', 'justice_lawyer_registration_nonce'],
  },
  {
    id: 'registration-paid-manual-form-state',
    name: 'Registration form preserves paid manual-invoice plan state',
    url: '/lawyer-registration/?plan_interest=lead_partner&payment_path=manual_invoice&utm_source=codex_check&utm_medium=live_funnel&utm_campaign=lawyer_acquisition&billing_first_name=Codex&billing_last_name=Lawyer&billing_phone=0501234567&billing_email=codex-lawyer@example.com&billing_legal_name=Codex+Legal+Ltd&billing_business_id=123456789&billing_invoice_email=billing-codex@example.com&billing_invoice_address=Tel+Aviv',
    type: 'page',
    required: [
      ...
      'name="billing_legal_name" value="Codex Legal Ltd"',
      'name="billing_business_id" value="123456789"',
      'name="billing_invoice_email" value="billing-codex@example.com"',
      'name="billing_invoice_address" value="Tel Aviv"',
  ...
  try {
    const response = await fetch(absoluteUrl(pathOrUrl), {
      ...
      headers: {
        ...
        'User-Agent': 'Jus-Tice-Lawyer-Revenue-Funnel-Checker/2.0',
      },
    });
...
async function runCheck(check) {
  ...
  const httpOk = check.redirect === 'manual' ? response.status >= 300 && response.status < 400 : response.ok;
...
function toMarkdown(results) {
  ...
  const lines = [
    `# Lawyer Acquisition Funnel Live Check - ${RUN_DATE}`,
    '',
    `- Status: ${statusLabel}`,
    `- Started: ${STARTED_AT}`,
    `- Base URL: ${BASE_URL}`,
    ...
    '- Homepage rule: the public homepage must stay legal-help/user-first; lawyer acquisition is allowed only as a quiet secondary path.',
    ...
    '',
    reviewCount === 0
      ? 'The public lawyer acquisition path is currently wired without returning the homepage to internal revenue-plan language: legal-help entry points remain present, the lawyer path is secondary, paid intent is routed to manual invoice activation, the paid registration form preserves manual-invoice plan state, the post-registration handoff avoids fake automatic-charge language, the dashboard gate exposes the next revenue step, and tracking assets are present.'
      : 'At least one public lawyer acquisition path marker was missing or unexpected. Review the table before relying on the live funnel for outreach.',
    '',
    '## Rerun',
    '',
    '```powershell',
