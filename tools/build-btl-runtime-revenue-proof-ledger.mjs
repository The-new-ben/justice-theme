...
function buildLedgerRows({ readiness, activation }) {
  ...
  const routeRows = [
    ...
    },
    {
      ...
      forbidden_action: 'Do not mark paid, claim revenue or automate payment collection without private payment evidence.',
    ...
    },
    {
      ...
      admin_location: adminLocations.billing,
      required_evidence: 'Payment proof exists through a private payment evidence URL and status is paid; invoice/reference alone is invoice-stage evidence.',
      crm_fields_or_anchors: 'qualified_lead_billing_status=paid; qualified_lead_payment_evidence_url present',
      pass_condition: 'Paid status has proof. The revenue loop can be counted once, with notes retained.',
      blocked_until: runtimeGate.next_action || 'Owner/admin records invoice/reference and private payment evidence.',
      allowed_next_action: 'Record first paid-lead achievement and decide whether to scale the Bituach Leumi loop.',
      forbidden_action: 'Do not count revenue if paid status lacks a private payment evidence URL.',
    ...
    },
    {
      ...
      required_evidence: '3 verified/routable specialists, 1 consented routed lead, invoice/reference, private payment evidence and no unresolved complaint/refund/ethics issue.',
