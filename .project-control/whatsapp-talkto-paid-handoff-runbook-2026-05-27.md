# WhatsApp / TalkTo Paid Handoff Runbook - 2026-05-27

Status: PRIVATE_RUNBOOK_VERIFIED_NOT_APPROVED_FOR_AUTOMATION
...
- Static source checks passing: 9/9
- Live automation approved: 0
- Public changes approved: 0

## Operator Rule

The safe path is: private CRM lead -> consent evidence -> routing hold -> no-PII partner preview -> accepted terms and billing contact -> owner release -> manual handoff -> invoice/reference for invoice_sent and private payment evidence for paid. Skip none of these steps.

## Static Source Checks

| ID | Gate | Status | File | Evidence | Markers | Missing | Next Action |
| --- | --- | --- | --- | --- | --- | --- | --- |
| WT-SRC-01 | manual_whatsapp_talkto_capture_bridge | PASS | inc/lead-crm.php | Manual admin bridge captures source, handoff path and keeps routing held by default. | 5/5 | - | Use wp-admin -> Justice CRM manual bridge only after owner approval for real lead creation. |
| WT-SRC-02 | legacy_import_defaults_to_repermission | PASS | inc/lead-crm.php | Bulk/legacy imports dedupe rows and default old leads to re-permission with routing hold. | 5/5 | - | Start with a small owner-approved export batch and keep routeable consent downgraded until evidence is reviewed. |
| WT-SRC-03 | permission_queue_and_routeable_consent | PASS | inc/lead-crm.php | CRM has explicit/owner-verified consent states plus do-not-contact handling. | 5/5 | - | Use only explicit_match_consent or owner_verified_consent before partner preview or routing. |
| WT-SRC-04 | router_blocks_unsafe_external_leads | PASS | inc/lead-routing.php | Router refuses held/manual external leads unless consent status and consent flag are both routeable. | 6/6 | - | Do not remove routing_hold until permission, partner terms and owner release are recorded. |
| WT-SRC-05 | no_pii_partner_preview_and_terms | PASS | inc/lead-crm.php | Partner preview and terms queue exists before PII release or paid handoff. | 5/5 | - | Send only anonymized facts until partner accepted terms, fee and billing contact. |
| WT-SRC-06 | final_owner_release_required | PASS | inc/lead-crm.php | Owner release queue requires client permission, partner terms and manual-only confirmation. | 5/5 | - | Record owner release deliberately; the system still sends nothing automatically. |
| WT-SRC-07 | qualified_lead_invoice_payment_proof | PASS | inc/lead-crm.php | Billing queue, badges and revenue summaries separate invoice references from payment evidence and prevent paid counts without an evidence URL. | 9/9 | - | Count revenue only after private payment evidence is present; invoice/reference alone can support invoice sent. |
| WT-SRC-08 | no_pii_audit_export | PASS | inc/lead-crm.php | Owner can export no-PII readiness rows without leaking client contact data or payment URLs. | 5/5 | - | Use audit export before bulk lead handoff, supplier/lawyer routing or invoice chase. |
| WT-SRC-09 | webhook_not_live_until_provider_gates | PASS | inc/lead-crm.php | Connector panel keeps webhook ingestion explicitly not-live until official provider controls are approved. | 5/5 | - | Use official WhatsApp/TalkTo provider routes only; no scraping, login automation or bypassing platform protections. |
...
| GATE-05 | safety_gate | billing_proof | REQUIRED_BEFORE_REVENUE_CLAIM | qualified_lead_billing_status, invoice reference for invoice_sent, payment evidence URL for paid, and owner note are stored. | Claim invoice sent from reference; claim paid only from payment evidence. | Do not mark paid, claim paid revenue, or activate paid status without private payment evidence. | inc/lead-crm.php qualified lead billing queue | HAD-76; HAD-87 | Use manual invoice/payment flow until Grow/Morning provider setup is fully verified. |
