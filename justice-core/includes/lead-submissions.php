 ...
 */
function justice_core_register_lead_cpt() {
 ...
 */
function justice_core_register_lead_meta() {
	$fields = array(
		'visitor_name'         => 'string',
...
add_action( 'init', 'justice_core_register_lead_meta' );

/**
 * Handle lead form submission.
 */
function justice_core_handle_lead() {
...
add_action( 'admin_post_justice_submit_lead', 'justice_core_handle_lead' );
add_action( 'admin_post_nopriv_justice_submit_lead', 'justice_core_handle_lead' );

/**
 * Admin columns for leads.
 */
function justice_core_lead_admin_columns( $columns ) {
	return array(
		'cb'          => $columns['cb'],
		'title'       => $columns['title'],
		'lead_phone'  => 'טלפון',
...
add_filter( 'manage_justice_lead_posts_columns', 'justice_core_lead_admin_columns' );

/**
 * Populate lead columns.
 */
function justice_core_lead_column_content( $column, $post_id ) {
	switch ( $column ) {
		case 'lead_phone':
...
add_action( 'manage_justice_lead_posts_custom_column', 'justice_core_lead_column_content', 10, 2 );

/**
 * Lead detail meta box.
 */
function justice_core_lead_meta_boxes() {
	add_meta_box( 'justice_lead_details', 'פרטי הליד', 'justice_core_lead_details_box', 'justice_lead', 'normal', 'high' );
}
add_action( 'add_meta_boxes', 'justice_core_lead_meta_boxes' );

function justice_core_lead_details_box( $post ) {
...
function justice_core_save_lead_status( $post_id ) {
...
add_action( 'save_post_justice_lead', 'justice_core_save_lead_status' );
