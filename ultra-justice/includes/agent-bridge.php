...
function uj_register_agent_routes() {
	register_rest_route( 'ultra-justice/v1', '/health', array(
		'methods' => 'GET', 'callback' => 'uj_agent_health', 'permission_callback' => 'uj_agent_permission',
	) );
	register_rest_route( 'ultra-justice/v1', '/theme/files', array(
		'methods' => 'GET', 'callback' => 'uj_agent_list_theme_files', 'permission_callback' => 'uj_agent_permission',
	) );
	register_rest_route( 'ultra-justice/v1', '/theme/file', array(
		'methods' => 'GET', 'callback' => 'uj_agent_read_theme_file', 'permission_callback' => 'uj_agent_permission',
		'args' => array( 'path' => array( 'required' => true, 'sanitize_callback' => 'sanitize_text_field' ) ),
	) );
	register_rest_route( 'ultra-justice/v1', '/theme/file', array(
		'methods' => 'POST', 'callback' => 'uj_agent_write_theme_file', 'permission_callback' => 'uj_agent_permission',
		'args' => array( 'path' => array( 'required' => true, 'sanitize_callback' => 'sanitize_text_field' ), 'content' => array( 'required' => true ) ),
	) );
}
add_action( 'rest_api_init', 'uj_register_agent_routes' );

function uj_agent_health() {
	return array( 'ok' => true, 'site' => home_url(), 'wp' => get_bloginfo( 'version' ), 'php' => PHP_VERSION, 'theme' => wp_get_theme()->get( 'Name' ), 'plugin' => uj_VERSION );
}

function uj_allowed_extensions() {
	return array( 'php', 'css', 'js', 'json', 'svg', 'txt', 'md' );
}

function uj_normalize_path( $rel ) {
	$rel = ltrim( str_replace( '\\', '/', $rel ), '/' );
	if ( false !== strpos( $rel, '..' ) ) { return false; }
	$ext = strtolower( pathinfo( $rel, PATHINFO_EXTENSION ) );
	if ( ! in_array( $ext, uj_allowed_extensions(), true ) ) { return false; }
...
function uj_agent_list_theme_files() {
	...
	foreach ( $rii as $f ) {
		...
		if ( in_array( $ext, uj_allowed_extensions(), true ) ) {
...
function uj_agent_read_theme_file( WP_REST_Request $r ) {
...
function uj_agent_write_theme_file( WP_REST_Request $r ) {
	...
	uj_agent_log( 'write', array( 'path' => $r->get_param( 'path' ), 'user' => get_current_user_id() ) );
	return array( 'ok' => true, 'bytes' => $bytes );
}

function uj_agent_log( $action, $data = array() ) {
