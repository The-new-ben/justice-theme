 ...
 *
 * Two modes:
 * 1. Admin-init auto-seed: creates 10 demo lawyers once (justice_core_seeded_v4 flag).
...
add_action( 'admin_init', 'justice_core_maybe_auto_seed' );

function justice_core_maybe_auto_seed(): void {
	if ( get_option( 'justice_core_seeded_v4' ) ) {
		return;
	}
	...
	update_option( 'justice_core_seeded_v4', true );
	justice_core_log( 'seeder', 'Auto-seeded 10 lawyer profiles (v4).' );
}

// ─── CSV Seeder (from project-control/lawyer-seed.csv) ───────────────────────

function justice_core_seed_lawyer_profile( array $row ): int|WP_Error {
	$existing = get_posts( array(
		'post_type'      => 'justice_lawyer',
		'title'          => $row['name'],
		'posts_per_page' => 1,
...
function justice_core_run_seed(): array {
	...
	fclose( $handle );
	justice_core_log( 'seeder_csv', 'CSV seed completed.', $results );
	return $results;
}
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'justice-core/v1', '/seed-lawyers', array(
		'methods'             => 'POST',
		'callback'            => function () {
			return new WP_REST_Response( justice_core_run_seed(), 200 );
		},
	...
	register_rest_route( 'justice-core/v1', '/seed-reset', array(
		'methods'             => 'POST',
		'callback'            => function () {
			delete_option( 'justice_core_seeded_v4' );
			justice_core_log( 'seeder_reset', 'Seed flag cleared. Will re-seed on next admin_init.' );
