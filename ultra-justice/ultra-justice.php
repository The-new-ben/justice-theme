...
// ─── Constants ──────────────────────────────────────────────────────────────
define( 'uj_VERSION', '1.0.0' );
define( 'uj_DIR',     plugin_dir_path( __FILE__ ) );
define( 'uj_URL',     plugin_dir_url( __FILE__ ) );
...
foreach ( $uj_includes as $file ) {
	$path = uj_DIR . $file;
	if ( file_exists( $path ) ) {
		require_once $path;
	} else {
		error_log( "[Ultra Justice] Missing include: {$path}" );
	}
}

// ─── Activation Hook ────────────────────────────────────────────────────────
register_activation_hook( __FILE__, 'uj_activate' );
function uj_activate(): void {
	$fns = array(
		'uj_register_articles_cpt',
		'uj_register_lawyer_cpt',
		'uj_register_lead_cpt',
		'uj_register_legal_tool_cpt',
		'uj_register_legal_request_cpt',
		'uj_register_practice_areas_taxonomy',
		'uj_register_city_taxonomy',
	);
	foreach ( $fns as $fn ) {
	...
	if ( function_exists( 'uj_log' ) ) {
		uj_log( 'activated', 'Ultra Justice v1.0.0 activated.' );
	}
}

// ─── Deactivation Hook ──────────────────────────────────────────────────────
register_deactivation_hook( __FILE__, 'uj_deactivate' );
function uj_deactivate(): void {
	flush_rewrite_rules();
}
