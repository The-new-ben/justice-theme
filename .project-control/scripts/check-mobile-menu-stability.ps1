...
Add-Check $checks "deploy_marker_current" (Contains-All -Text $texts.functions -Tokens @(
	"1.1.72",
	"2026-05-28-lawyer-registration-revenue-bridge-v1"
)) "Theme version and deployment marker must point to the current lawyer-registration revenue-bridge release." $files.functions

Add-Check $checks "premium_css_cache_bumped" ($texts.enqueue.Contains("'4.5.8'")) "Premium stylesheet cache version must be bumped for deployment." $files.enqueue
