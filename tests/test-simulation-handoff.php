...
jt_handoff_assert( false !== strpos( $criminal, 'scenario=investigation-rehearsal' ), 'Canonical product scenario is missing.' );
...
jt_handoff_assert( false !== strpos( $family_bridge, 'scenario=mediation-preparation' ), 'Practice area lost its canonical product scenario.' );

$commercial_journeys = array(
	'family-law'            => array( 'family-law', '/divorce-lawyer/', 'mediation-preparation' ),
	'criminal-law'          => array( 'criminal-law', '/criminal-defense-attorney/', 'investigation-rehearsal' ),
	'real-estate-law'       => array( 'real-estate', '/articles/real-estate-attorney/', 'transaction-dispute-rehearsal' ),
	'medical-malpractice'   => array( 'medical-malpractice', '/medical-malpractice-lawyer/', 'medical-expert-preparation' ),
);
foreach ( $commercial_journeys as $area => $expected ) {
	...
	jt_handoff_assert( $expected[2] === $context['scenario'], 'Commercial area received the wrong scenario: ' . $area );
	...
	jt_handoff_assert( false !== strpos( $url, 'scenario=' . rawurlencode( $expected[2] ) ), 'Commercial bridge lost its scenario: ' . $area );
}
