...
add_shortcode( 'justice_brain', function () {
	...
	return '<p>שלוש שאלות קצרות, בלי פרטים אישיים, ובסוף: פנייה מסודרת בוואטסאפ או בטופס, כבר עם כל ההקשר.</p>' . $trust
		...
		. '<p><label for="jtb-name">שם מלא</label><input id="jtb-name" type="text" name="lead_name" autocomplete="name" required></p>'
		. '<p><label for="jtb-phone">טלפון</label><input id="jtb-phone" type="tel" name="lead_phone" autocomplete="tel" required></p>'
		. '<p><label for="jtb-city">עיר</label><input id="jtb-city" type="text" name="lead_city" autocomplete="address-level2"></p>'
