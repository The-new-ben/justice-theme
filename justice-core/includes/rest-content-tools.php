...
add_action( 'rest_api_init', 'justice_core_register_content_routes' );

function justice_core_register_content_routes(): void {
	$admin_only = function () {
	...
	register_rest_route( 'justice-core/v1', '/content/lawyers', array(
		'methods'             => 'GET',
		'callback'            => 'justice_core_content_lawyers',
	...
	register_rest_route( 'justice-core/v1', '/content/posts', array(
		'methods'             => 'GET',
		'callback'            => 'justice_core_content_posts',
		'permission_callback' => $admin_only,
		'args'                => array(
			'per_page' => array( 'type' => 'integer', 'default' => 50, 'minimum' => 1, 'maximum' => 200 ),
			'page'     => array( 'type' => 'integer', 'default' => 1, 'minimum' => 1 ),
	...
	register_rest_route( 'justice-core/v1', '/content/leads', array(
		'methods'             => 'GET',
		'callback'            => 'justice_core_content_leads',
		'permission_callback' => $admin_only,
	) );
	...
	register_rest_route( 'justice-core/v1', '/content/update-meta', array(
		'methods'             => 'POST',
		'callback'            => 'justice_core_update_meta',
		'permission_callback' => $admin_only,
		'args'                => array(
	...
	register_rest_route( 'justice-core/v1', '/content/trash-post', array(
		'methods'             => 'POST',
		'callback'            => 'justice_core_trash_post',
		'permission_callback' => $admin_only,
		'args'                => array(
			'post_id' => array( 'type' => 'integer', 'required' => true ),
...
function justice_core_content_lawyers( WP_REST_Request $request ): WP_REST_Response {
...
function justice_core_content_posts( WP_REST_Request $request ): WP_REST_Response {
...
function justice_core_content_leads( WP_REST_Request $request ): WP_REST_Response {
	$query = new WP_Query( array(
 ...
 */
function justice_core_update_meta( WP_REST_Request $request ): WP_REST_Response {
	...
	justice_core_log( 'update_meta', "Updated {$meta_key} on post #{$post_id}", array(
		'post_id'  => $post_id,
		'meta_key' => $meta_key,
		'old_val'  => $old_val,
		'new_val'  => $meta_val,
 ...
 */
function justice_core_trash_post( WP_REST_Request $request ): WP_REST_Response {
	$post_id = absint( $request->get_param( 'post_id' ) );
	...
	justice_core_log( 'trash_post', "Trashed post #{$post_id}: {$post->post_title}", array(
		'post_id' => $post_id,
		'title'   => $post->post_title,
		'type'    => $post->post_type,
		'reason'  => $reason,
