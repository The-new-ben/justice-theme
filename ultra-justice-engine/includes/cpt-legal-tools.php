...
function uje_register_legal_tool_cpt(): void {
...
add_action( 'init', 'uje_register_legal_tool_cpt' );

function uje_register_legal_request_cpt(): void {
...
add_action( 'init', 'uje_register_legal_request_cpt' );

function uje_register_legal_tool_meta(): void {
	$tool_fields = array(
		'tool_type'              => 'string',
...
add_action( 'init', 'uje_register_legal_tool_meta' );

function uje_seed_legal_tools_if_needed(): void {
	if ( ! current_user_can( 'manage_options' ) || ! (bool) apply_filters( 'ultra_justice_engine_enable_legal_tools_seed', false ) || get_option( 'uje_legal_tools_seeded_v1' ) ) {
		return;
	}
	...
	update_option( 'uje_legal_tools_seeded_v1', 1, false );
}
add_action( 'admin_init', 'uje_seed_legal_tools_if_needed' );

function uje_handle_legal_tool_request(): void {
...
add_action( 'admin_post_justice_submit_legal_request', 'uje_handle_legal_tool_request' );
add_action( 'admin_post_nopriv_justice_submit_legal_request', 'uje_handle_legal_tool_request' );
